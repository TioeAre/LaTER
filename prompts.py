...
"""
        ...
        elif args.task in ["arc_easy", "arc_challenge", "gpqa", "medqa", "commonsense_qa"]:
            user_prompt = f"""
Target Question: {question}
...
def build_agent_messages_hierarchical_text_mas(role: str, question: str, context: str = "", method=None, args=None):
    ...
    if args.task in ["gsm8k", "aime2024", "aime2025", "math500", "prosqa"]:
        if role == "planner":
            user_content = f"""
You are a math agent. Given the final answer inside \\boxed{{YOUR_FINAL_ANSWER}}.
...
def build_agent_messages_single_agent(question: str, args=None):
    ...
    assert args.method in ["baseline", "latent_switch", "latent_qwen3"], "this prompt only for single-agent methods"
    # assert "qwen" in args.model_name.lower(), "this prompt only for qwen models"
...
def build_latentswitch_messages_latent_think(question: str, args=None):

    system_message = "You are a helpful assistant."

    assert args.method in ["baseline", "latent_switch"], "this prompt only for baseline method (single agent)"
    # assert "qwen" in args.model_name.lower(), "this prompt only for qwen models"
    ...
    if task in ["gsm8k", "aime2024", "aime2025", "math500", "prosqa"]:
        user_content = f"""
You are a helpful assistant. Given an input question, think about what methods should be used and corresponding detailed step-by-step plan to solve the question.
